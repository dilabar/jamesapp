...
`;document.documentElement.insertAdjacentHTML('beforeend',themesettings);setTimeout(function(){const applyScriptAfterDelay=()=>{const themeRadios=document.querySelectorAll('input[name="theme"]');const sidebarRadios=document.querySelectorAll('input[name="sidebar"]');const colorRadios=document.querySelectorAll('input[name="color"]');const sidebarBgRadios=document.querySelectorAll('input[name="sidebarbg"]');const resetbutton=document.getElementById('resetbutton');function setThemeAndSidebarTheme(theme,sidebarTheme,color,sidebarBg){document.documentElement.setAttribute('data-theme',theme);document.documentElement.setAttribute('data-sidebar',sidebarTheme);document.documentElement.setAttribute('data-color',color);document.documentElement.setAttribute('data-sidebar-bg',sidebarBg);localStorage.setItem('theme',theme);localStorage.setItem('sidebarTheme',sidebarTheme);localStorage.setItem('color',color);localStorage.setItem('sidebarBg',sidebarBg);}
function handleInputChange(){const theme=document.querySelector('input[name="theme"]:checked')?.value||'light';const sidebarTheme=document.querySelector('input[name="sidebar"]:checked')?.value||'light';const color=document.querySelector('input[name="color"]:checked')?.value||'red';const sidebarBg=document.querySelector('input[name="sidebarbg"]:checked')?.value||'sidebarbgnone';setThemeAndSidebarTheme(theme,sidebarTheme,color,sidebarBg);}
themeRadios.forEach((radio)=>{radio.addEventListener('change',handleInputChange);});sidebarRadios.forEach((radio)=>{radio.addEventListener('change',handleInputChange);});colorRadios.forEach((radio)=>{radio.addEventListener('change',handleInputChange);});sidebarBgRadios.forEach((radio)=>{radio.addEventListener('change',handleInputChange);});if(resetbutton){resetbutton.addEventListener('click',function(){setThemeAndSidebarTheme('light','light','red','sidebarbgnone');themeRadios[0].checked=true;sidebarRadios[0].checked=true;colorRadios[0].checked=true;sidebarBgRadios[0].checked=true;});}
const savedTheme=localStorage.getItem('theme')||'light';const savedSidebarTheme=localStorage.getItem('sidebarTheme')||'light';const savedColor=localStorage.getItem('color')||'red';const savedSidebarBg=localStorage.getItem('sidebarBg')||'sidebarbgnone';setThemeAndSidebarTheme(savedTheme,savedSidebarTheme,savedColor,savedSidebarBg);const themeRadio=document.querySelector(`input[name="theme"][value="${savedTheme}"]`);const sidebarRadio=document.querySelector(`input[name="sidebar"][value="${savedSidebarTheme}"]`);const colorRadio=document.querySelector(`input[name="color"][value="${savedColor}"]`);const sidebarBgRadio=document.querySelector(`input[name="sidebarbg"][value="${savedSidebarBg}"]`);if(themeRadio)themeRadio.checked=true;if(sidebarRadio)sidebarRadio.checked=true;if(colorRadio)colorRadio.checked=true;if(sidebarBgRadio)sidebarBgRadio.checked=true;};applyScriptAfterDelay();},500);